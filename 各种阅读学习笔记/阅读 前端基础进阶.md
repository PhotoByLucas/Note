...
2. 变量对象
   ...
   - 创建
     ...
     1. 建立 arguments 对象。检查当前上下文中的参数，建立该对象下的属 与属性值。
...
2. 如果函数独立调用，那么该函数内部的 this，则指向 undefined。
...
- 任务队列分为
  ...
  2. micro-task（微任务）=> job  
     process.nextTick, Promise, Object.observe(已废弃), MutationObserver(html5 新特性)

- 执行方法：
  1. 读到 task 会形成一个左边的队列
  2. 读到 job 会形成一个右边的队列，job 附属于某一个 task 任务中，当一个 task 执行完毕后，执行下一个 task
  3. 相同的 task/job 会形成一个任务**队列**，按照先进先出的顺序执行。  
