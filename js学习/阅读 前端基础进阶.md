...
5. 原型链  
> add()是Function对象的实例。而Function的原型对象同时又是Object原型的实例。   
...
+ js为单线程，事件循环是唯一的，但是**任务队列**可以拥有多个。
+ 任务队列分为  
...
+ 执行方法：
    1. 读到task会形成一个左边的队列
    2. 读到job会形成一个右边的队列，job附属于某一个task任务中，当一个task执行完毕后，执行下一个task
    3. 相同的task/job会形成一个任务**队列**，按照先进先出的顺序执行。  
    如：两个settimeout会根据其时间值，形成一个队列  
    timeout1 -> timeout2
## Promise
